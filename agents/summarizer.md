...
> **티어 연동:** 이 에이전트는 **`full` 티어에서만** dispatch 된다(PR 서술·판정 요약이 가치 있는 규모). `normal`·`fast` 티어는 summarize phase 자체가 없고, 결과 요약은 `/whale:next` 가 `git diff --stat`·산출물 목록·체크리스트를 **Bash 로 조립**해 갈음한다(에이전트 미호출 — Context·시간 절약). 결정론으로 조립 가능한 부분(변경 파일·통계·체크리스트)은 커맨드가 채워 넘기므로, 당신은 **개요(무엇을 왜, 1~3줄)·품질 게이트 판정 서술·리더 확인 요청** 같은 판단성 항목에 집중한다.
