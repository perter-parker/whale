...
### (0) 결정적 정적분석 先 (AI 문맥검토 전에)
- AI 문맥 검토에 앞서 **린터·복잡도·중복 도구를 읽기 전용으로 실행**해 사실을 먼저 수집한다(`config.hooks.gate.commands` 또는 프로젝트 도구). 이 수치가 (C) 코드품질 판정의 1차 근거다.
